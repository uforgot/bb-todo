...
| `BB_VOICE_CHANNEL_IDS` | 감시 채널 id (콤마 구분), 기본 bb-private |
...
발동하면 다음 두 가지 흐름 중 하나:
- **Discord reply(댓글)로 봇 메시지에 답글 단 경우** → 그 봇으로 직행
- **일반 무멘션** → 채널 직전 메시지부터 거슬러 올라가며 `voice-config.json`
  bots에 등록된 봇을 찾아 그 봇으로 relay (face listener 같은 미등록 봇은
  스킵)

### relay 페이로드

listener 봇이 사용자 메시지에 `msg.reply()`로 댓글을 단다 (진짜 Discord
threaded reply). 본문은:

```
<@bot_id> [Relayed] User didn't mention you directly. Read the referenced
message (text + images) and reply naturally.
```

원본 메시지의 첨부(이미지 등)는 그대로 재첨부해서 같이 박는다 — agent
harness가 reply reference를 따라가서 이미지를 가져오지 못하는 케이스를
커버하기 위함.
...
| 변수 | 용도 |
|---|---|
| `RELAY_PROMPT` | relay 본문의 지시 문구 override (기본은 위 영어 문구) |

webhook 없이 봇 client의 `msg.reply`만 사용하므로 별도 webhook env 필요
없음.

### 루프 안전성

- relay reply는 listener author → 다른 핸들러들이 `msg.author.bot` 체크로
  스킵
