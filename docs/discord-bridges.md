...
기본값은 비활성화다. `RELAY_FOLLOWUP_ENABLED=true`일 때만 아래 자동
relay가 동작한다. thread에서도 동작할 수 있으므로 Hermes/OpenClaw의
thread 자동응답 설정과 같이 확인해야 한다.
...
- 텍스트 또는 이미지 첨부 있음
- 등록된 봇 멘션이 본문에 없음
- 바로 직전 메시지가 등록된 봇의 답변이고, 3시간 이내임
...
listener 봇이 댓글(reply)을 달지 않고 새 일반 메시지를 보낸다. 본문은
target mention, relay instruction, 원사용자 context, 원문으로 나뉜 envelope다:
...
original_user_name=...
original_message_id=...
previous_bot_message_id=...

[user_content]
원문 텍스트
```

원본 메시지의 첨부(이미지 등)는 그대로 재첨부한다. agent는 listener bot을
실제 화자로 취급하지 않고, `[user_content]`만 사용자 발화로 처리해야 한다.

### 환경 변수

| 변수 | 용도 |
|---|---|
| `RELAY_FOLLOWUP_ENABLED` | `true`일 때만 무멘션 followup relay 활성화. 기본값 비활성화 |
| `RELAY_CHANNEL_IDS` | relay 감시 채널 id (콤마 구분). 없으면 `BB_VOICE_CHANNEL_IDS`를 사용. thread는 parent channel id 기준으로도 매칭 |
| `RELAY_ALL_VISIBLE_CHANNELS` | `true`면 listener 봇이 볼 수 있는 모든 채널/thread에서 relay 허용 |
...
분리되어 있다. relay를 여러 Discord 채널에서 쓰더라도, voice 응답
