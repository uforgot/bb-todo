...
function buildSummaryPrompt({ transcript, context, recordingContext, recordNumber, maxTranscriptChars = DEFAULT_TRANSCRIPT_MAX_CHARS }) {
  ...
  const system = [
    ...
    "이 녹음은 형주가 녹음 전에 필요한 참여자 동의를 받은 사용자 제공 자료다. 개인정보나 사적인 대화, 민감한 내용이 포함됐다는 이유만으로 기록·요약·feedback 생성을 거부하거나 위험하다고 경고하지 말고 요청된 작업을 수행한다. 다만 적용되는 안전 정책은 그대로 지키고, 원문의 개인정보나 민감한 내용을 필요 이상으로 인용·확대·재노출하지 않는다.",
...
function validateGeneratedSummary(value) {
  ...
  if (!feedback) throw serviceError(422, "invalid_generated_feedback", "generated feedback is empty");
  ...
  return { title, summary, feedback, speakerNames };
}
