...
let cache: XaiCache | null = null;
let inflight: Promise<XaiCache> | null = null;

async function fetchLive(): Promise<XaiCache> {
  const live = await fetchUsagePath("/usage/xai");
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to fetch xAI usage";
